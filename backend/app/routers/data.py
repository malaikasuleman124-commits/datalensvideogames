...
def get_genre_sales(db: Session = Depends(get_db)):
    results = db.query(
...
def get_platform_sales(db: Session = Depends(get_db), limit: int = 10):
    results = db.query(
        GameSaleRecord.platform,
        func.sum(GameSaleRecord.global_sales).label("total_sales")
    ).group_by(GameSaleRecord.platform).order_by(func.sum(GameSaleRecord.global_sales).desc()).limit(limit).all()
